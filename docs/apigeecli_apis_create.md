...
```
  ...
  -n, --name string                                   API Proxy name
      --oas-google-accesstoken-scope-literal string   Generate Google Access token with target endpoint and set scope
      --oas-google-idtoken-aud-literal string         Generate Google ID token with target endpoint and set audience
      --oas-google-idtoken-aud-ref string             Generate Google ID token token with target endpoint and set audience reference
      --oas-target-url-ref string                     Set a reference variable containing the target endpoint
